...
if (isset($_POST['enviar_mensagem'])) {
    ...
    if (empty($titulo) || empty($mensagem)) {
        ...
        $sql = "INSERT INTO forumciencias (num_aluno, nome_autor, titulo, mensagem, parent_id)
                VALUES ('$num_aluno', '$nome_autor', '$titulo', '$mensagem', $parent_value)";
       
        if ($conn->query($sql) === TRUE) {
            
            header("Location: forumciencias.php?sucesso=" . urlencode("Mensagem publicada com sucesso!"));
            exit();
        } else {
...
if (isset($_GET['excluir'])) {
    ...
    $sql = "SELECT * FROM forumciencias WHERE id = $id AND num_aluno = '$num_aluno'";
    $resultado = $conn->query($sql);
   
    if ($resultado->num_rows === 1) {
     
        $sql = "DELETE FROM forumciencias WHERE id = $id";
        if ($conn->query($sql) === TRUE) {
            header("Location: forumciencias.php?sucesso=" . urlencode("Mensagem excluída com sucesso!"));
            exit();
        } else {
...
if (isset($_POST['editar_mensagem'])) {
    ...
    if ($resultado->num_rows === 1) {
        ...
        $sql = "UPDATE forumciencias SET titulo = '$titulo', mensagem = '$mensagem' WHERE id = $id";
        if ($conn->query($sql) === TRUE) {
            header("Location: forumciencias.php?sucesso=" . urlencode("Mensagem atualizada com sucesso!"));
            exit();
        } else {
...
$sql = "SELECT * FROM forumciencias WHERE $where_clause ORDER BY data_criacao DESC";
$mensagens_principais = $conn->query($sql);
 

function buscarRespostas($conn, $parent_id) {
    $sql = "SELECT * FROM forumciencias WHERE parent_id = $parent_id ORDER BY data_criacao ASC";
...
<body>
    ...
    <div>
        <a href="forumciencias.php?editar=<?php echo $mensagem['id']; ?>"
           class="btn btn-small">
            <i class="fas fa-edit"></i> Editar
        </a>
        <a href="forumciencias.php?excluir=<?php echo $mensagem['id']; ?>"
...
<?php endif; ?>
    ...
    <div>
        <a href="forumciencias.php?editar=<?php echo $resposta['id']; ?>"
           class="btn btn-small">
            <i class="fas fa-edit"></i> Editar
        </a>
        <a href="forumciencias.php?excluir=<?php echo $resposta['id']; ?>"
           class="btn btn-danger btn-small"
...
<div class="edit-form-overlay">
    <div class="edit-form-container">
        ...
        <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" class="edit-form">
            ...
            <div class="form-actions">
                <a href="forumciencias.php" class="btn btn-danger">Cancelar</a>
